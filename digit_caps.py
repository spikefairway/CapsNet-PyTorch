...
class DigitCaps(nn.Module):
    def __init__(self, routing_iters=3,
                 num_in_capsules=1152, in_capsule_size=8,
                 num_out_capsules=10, out_capsule_size=16):
        super(DigitCaps, self).__init__()

        self.routing_iters = routing_iters

        self.in_capsules = num_in_capsules
        self.in_capsule_size = in_capsule_size
        self.out_capsules = num_out_capsules
    ...
    def forward(self, x):
        ...
        x = torch.stack([x] * self.out_capsules, dim=2).unsqueeze(4)
        # x: [batch_size, in_capsules=1152, out_capsules=10, in_capsule_size=8, 1]
        ...
        u_hat = torch.matmul(W, x) # matrix multiplication
        ...
        b_ij = Variable(torch.zeros(1, self.in_capsules, self.out_capsules, 1))
        if torch.cuda.is_available():
            b_ij = b_ij.cuda()
        # b_ij: [1, in_capsules=1152, out_capsules=10, 1]
        ...
        for iteration in range(self.routing_iters):
            ...
            if iteration == self.routing_iters - 1:
                ...
                # c_ij * u_hat: [batch_size, in_capsules=1152, out_capsules=10, out_capsule_size=16, 1]
                ...
                u_vj1 = torch.matmul(u_hat_detached.transpose(3, 4), v_j).squeeze(4).mean(dim=0, keepdim=True)
                # u_vj1: [1, in_capsules=1152, out_capsules=10, 1]
